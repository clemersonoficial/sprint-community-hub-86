      ...
      <div className="container mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3 cursor-pointer" onClick={() => navigate("/")}>
            ...
            <img src="/lovable-uploads/76b9261e-f21a-48ca-ad98-ea9a42a52e30.png" alt="Logomarca" className="h-12 w-auto" />
          </div>

          {/* Desktop Navigation */}
          <nav className="hidden md:flex items-center space-x-2">
            {navigation.map(item => <Button key={item.name} variant="ghost" className={`btn-ghost ${location.pathname === item.href ? 'bg-primary/10' : ''}`} onClick={() => handleNavigation(item.href)}>
        ...
        {isMenuOpen && <nav className="md:hidden mt-4 pb-4 border-t border-border pt-4">
            <div className="flex flex-col space-y-2">
              {navigation.map(item => <Button key={item.name} variant="ghost" className="justify-start" onClick={() => handleNavigation(item.href)}>
